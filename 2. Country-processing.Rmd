...
total_change_df <- merged_data_subset %>%
  group_by(job,isco) %>%
  summarise(avg_change = weighted.mean(change, weight = COUNTRY)) 

# Calculate the yearly average change per occupation
year_change_df<- merged_data_subset |> 
  group_by(YEAR,job,isco) |> 
  summarise(avg_year = weighted.mean(change, weight=COUNTRY), 
            avg_isco = weighted.mean(ISCOPROP, weight=COUNTRY))

# Calculate the country average change per occupation
country_change_df <- merged_data_subset %>%
  group_by(COUNTRY,job,isco) %>%
  summarise(avg_cntry = weighted.mean(change, weight = COUNTRY)) 
...
### Cross-country differences in average employment change (2012-2021): Information and communications technology professionals

```{r}
library(stringr)
tec <- merged_data_subset |> 
  filter(str_detect(job, "Information and communications technology professionals")) |> 
...
map1<- ggplot() +
  ...
  scale_fill_gradient2(low = "#006d77", mid = "white", high = "#b70000", name = "Average Change",midpoint = 0) +
  theme_void()
```

### Cross-country differences in average employment change (2012-2021): Labourers in mining, construction, manufacturing and transportation.
...
map2<- ggplot() +
 ...
 scale_fill_gradient2(low = "#006d76", mid = "white", high = "#b70000", name = "Average Change", midpoint = 0) + 
  theme_void() 
```

```{r}
# Arrange together
ggarrange(map1,map2, ncol =2, nrow=1, legend = "bottom") 
```

### Occupational trends in the employment structure

**Changes in employment between routine and non-routine occupations over time.**

```{r}
agg_trend <- merged_data_subset |> 
   mutate(sector = ifelse(routine > 0.5, "Routine", "Non-routine ")) |> 
   group_by(YEAR, sector) %>%
  summarise(agg_avg_year = weighted.mean(avg_year, weight=COUNTRY)*100)  

ggplot(agg_trend, aes(x = YEAR, y = agg_avg_year,
                  color = as.factor(sector), group =sector)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  lims(y = c(-11,10)) +
  labs(title = "",
       subtitle = "Average change in employment",
       x = "",
       y = "" , 
       color = "Sector") +
  scale_fill_manual() + 
...
**Changes in employment between high and low ICT use occupations over time.**

```{r}
merged_data_subset |> 
   mutate(sector = ifelse(ICT > 0.5, "High ICT use", "Low ICT use")) |> 
   group_by(YEAR, sector) %>%
  summarise(agg_avg_year = weighted.mean(avg_year, weight=COUNTRY, na.rm = TRUE)*100) |> 
  ggplot(aes(x = YEAR, y = agg_avg_year,
                  color = factor(sector), group = sector)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  lims(y = c(-12,10)) +
  labs(title = "",
...
### Exploratory analysis of LFS variables

Average Ratio of Online Work by Occupation (2006-2021), weighted by country

```{r}
# Calculate weighted average ratio of online-work per occupation
 weighted_ratio <-merged_data %>%
  group_by(job,isco) %>%
  summarise(avg_ratio= weighted.mean(ONRATIO, weight = COUNTRY)) |> 
  arrange(desc(avg_ratio)) |> 
  na.omit()

# Visualization
ggplot(weighted_ratio, aes(x = reorder(job, avg_ratio), y = avg_ratio)) +
  geom_bar(stat = "identity") +
  labs(title = "Average Ratio of Online Work by Occupation (2006-2021)",
       x = "", y = "", caption = "Note: The presented averages are weighted by country. Cross-country averages are taken over the 16 countries included in the analysis.") +  coord_flip() + custom_theme() 
```

Average change in employment level across countries by occupation (2006-2021), weighted by country.

```{r}
weighted_change <- merged_data %>% group_by(job,isco) %>%summarise(avg_change = weighted.mean(change, weight = COUNTRY))

# Sort occupations by change (descending for most lost employment)
top_losers <- head(weighted_change %>% arrange(avg_change),10)
top_losers
top_gainers <- head(weighted_change %>% arrange(desc(avg_change)),10)
top_gainers
```

```{r}
my_colors <- c("Gained Employment" = "#FFF176", "Lost Employment" = "#C4BDF3")
# Visualization
ggplot(weighted_change, aes(x = reorder(job, avg_change), y = avg_change, fill = factor(avg_change < 0, labels = c("Gained Employment", "Lost Employment")))) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = my_colors) +
  labs(title = "Average change in employment level across countries by occupation (2006-2021)",
       x = "", y = "", caption = "Note: The presented averages are weighted by country. Cross-country averages are taken over the 16 countries included in the analysis.") + 
 custom_theme() +
  coord_flip() 
```
...
```{r}
# Arrange together
grid <- ggarrange(c1,c2,c3,c4,c5,c6,
          ncol =3, nrow=2) 
grid + ggtitle("") + custom_theme() 
```

```{r, eval=FALSE}
save_jpg <- function(plot, filename = "", width = 24, height = 12, unit = "cm", dpi = 300) {
  ggsave(
    plot = plot,
    filename = filename,
    device = 'jpg',
...
save_jpg(grid, filename = "corr.jpg")
```
...
prop_data <- merged_data|> 
  select(job,YEAR,avg_isco) |> 
  distinct()

# Pivot the data wider to create the matrix
hmprop <- prop_data %>%
    pivot_wider(names_from = YEAR, values_from = avg_isco) 
  
  # Arrange based on the specified year (e.g., 2021)
  hmprop <- hmprop %>%
    arrange(!!sym("2021")) 
  
  # Assign the rownames after arranging the heatmap 
  rownames <- hmprop$job
```

```{r}
# Plot the heatmap
  heatmap(as.matrix(hmprop[, -c(1)]), 
          Rowv = NA, 
          Colv = NA, 
          scale = "column", 
          main = paste("Yearly average proportion of employees by occupation"),
          xlab = "",
          ylab = "",
          cexCol = 1, 
          labRow = rownames)

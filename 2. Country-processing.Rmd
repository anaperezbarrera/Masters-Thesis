...
Cada fila en prop_data representa la proporción de empleados en una ocupación específica para un año específico.
...
year_change_df<- merged_data_subset |> 
  ...
  summarise(avg_year = weighted.mean(empchange, weight=COUNTRY)*100)
...
```{r}
 merged_data_subset <- merged_data_subset %>%
  mutate(sector = case_when(
    ...
    str_detect(job, "Information") ~ "Information Technology and Digital Communications",
    ...
    TRUE ~ "Construction, manufacturing and transport"
  )) 
```

### Sector employment rate variation across Europe

```{r}
# Calculate the yearly average change per sector

### Visualization
ggplot(sector_data, aes(x = YEAR, y = empsector, color = sector, group = sector)) +
  geom_line(size=1) + labs(x="", y="") + 
...
map2<- ggplot() +
  geom_sf(data= europe2, aes(fill = avg_country)) +
   scale_fill_gradientn(
    colors = c("darkgreen", "white", "#b70000"), 
  ...
  labs(title="")+ coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE)
```
...
```{r}
ggplot(merged_data_subset, aes(x = ICT, y = avg_change, 
                               color = ICT,label=job)) +
  geom_point(size = 1) +  
 geom_text(vjust = -1, hjust = 0.5, size = 3) +
...
new_data <- merged_data_subset |> 
  dplyr::select(-isco,-job,-EMPSTAT, -empchange, -avg_change,-avg_year) |> 
  distinct(.keep_all = TRUE)
...
```{r}
cor_data <-training |> dplyr::select(-COUNTRY,-YEAR,-sector)
...
model <- lm(formula2, data = training)
summary(model)
```
...
```{r}
ggplot(imp, aes(x = reorder(rownames(imp), Overall), y = Overall)) +
...
```{r}
ggplot(merged_data_subset, aes(x = reorder(as.factor(job),iscoprop), y = iscoprop)) +
  geom_boxplot() +
  theme_minimal() +
  coord_flip()+

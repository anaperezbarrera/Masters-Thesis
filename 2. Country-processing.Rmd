...
**The new variable :employment rate variation** is calculated using the iscoprop variable (proportion of employees by year and occupation) to measure the yearly variation (loss/gain)between 2006-2021.
...
```{r}
iscolong <-iscoag |> 
    pivot_longer(cols = starts_with(c("ICTadvanced")),
...
puntos <- ggplot(iscolong, aes(x = characteristic, 
                               y = job, size = values, color=values)) +
...
new_data <- merged_data_subset |> 
  dplyr::select(-isco,-job,-avg_change) |> 
  distinct(.keep_all = TRUE)
...
# Linear mixed model
library(lme4)
model <- lmer(iscoprop ~ sunratio + satratio + nightratio + shiftratio + extraratio + 
    supratio + ftratio + onratio + femratio + empchange + strength + 
    dexterity + navigation + uncodified + business + humanities + 
    numeracy + probsolving + infogath + creativity + planning + 
    social + serving + managing + autonomy + latitude + control + 
    team + repetitiv + standard + certainty + machines + ICT + 
    ICTbasic + ICTadvanced + (1 | COUNTRY), data = training)
summary(model)
```
...
```{r}
coef <- coef(for_tune$finalModel, for_tune$bestTune$nvmax)
```
...
lasso_tune <- train(formula, data = training,
                    method='lasso',
                    preProc=c('scale','center'),
                    tuneGrid = lasso_grid,
                    trControl=ctrl)
...
```{r, eval=FALSE}
partial(knn_tune, pred.var = "ICT", plot = TRUE, rug = TRUE)
```
...
xgb <- train(formula, data = training,
               method = "gbm",
               trControl = control,
               tuneGrid = grid,
               verbose = FALSE)
...
ggplot(merged_data_subset,
       mapping = aes(x = YEAR, y = ISCOPROP, color = COUNTRY)) +
  geom_point() +
...
```{r}
modelo <- lm(change ~ FEMRATIO + EXTRARATIO + strength + dexterity + navigation + uncodified + business + humanities + numeracy + probsolving + creativity + planning + social + serving + managing + autonomy + latitude + team + repetitiv + standard + certainty + ICT + ICTbasic + ICTadvanced + as.factor(COUNTRY)+as.factor(YEAR), data = datos_panel)

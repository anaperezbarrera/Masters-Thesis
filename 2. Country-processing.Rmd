...
```{r}
selected <- merged_data_subset |> filter(str_detect(job,c("Information|manufacturing")))

# Gráfico ggplot con etiquetas seleccionadas
ggplot(merged_data_subset, aes(x = ICTadvanced, y = avg_change, color = ICTadvanced)) +
  geom_point(size = 3) +
  geom_text(data = selected, aes(label = job), vjust = -1, hjust = 0.5, size = 3) +
  scale_color_gradient(low = "darkgreen", high = "red") + labs (x="",y="") + 
...
ggplot(sector_long, aes(x = characteristic, y = sector, size = values, color=values)) +
  geom_point(alpha = 1) +
  scale_size_continuous(range = c(0, 20), name = "") + 
  scale_color_gradientn(colours = new_palette) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "",
       x = "",
       y = "") + theme(axis.text.y = element_text(face = "bold", size=10),
